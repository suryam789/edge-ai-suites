...
void FastMappingNode::init()
{
  ...
  subscribers_.reset(new CameraSubscribers(
    this->shared_from_this(), frame_queue_,
    std::bind(&FastMappingNode::setIntrinsics, this, std::placeholders::_1)));
...
{
  octomap_->setIntrinsics(msg->height, msg->width, msg->k[2], msg->k[5], msg->k[0], msg->k[4]);
}
...
void FastMappingNode::getPoseFromTf(std::shared_ptr<ImageFrame> currFrame)
{
  ...
  try {
    transform = tf_buffer_->lookupTransform(
      config_.p_map_frame_, header.frame_id, tf2::TimePointZero,
      tf2::durationFromSec(config_.p_tf_delay_));
  } catch (tf2::TransformException & e) {
    std::string errstr(e.what());
    RCLCPP_ERROR(
      get_logger(), "Failed to get transform from %s to %s: %s", header.frame_id.c_str(),
      config_.p_map_frame_.c_str(), errstr.c_str());
...
void FastMappingNode::worker()
{
  ...
  while (rclcpp::ok()) {
    ...
    if (!frame_queue_.wait_pop(currFrame)) {
      break;
    }
...
void FastMappingNode::printStats()
{
  ...
  if (dur > interval) {
    ...
    RCLCPP_INFO(
      get_logger(),
      "fast_mapping got %d images in %.1fs. Aligned %d. Processed %d (%.2lf Hz). %ld left in queue",
      nimage, dur.seconds(), aligned_pf, processed, processed / dur.seconds(), queue_length);
  } else {
    stat_mutex_.unlock();
  }
}
