...
public:
  ...
  inline void finish()
  {
    ...
    // Format result here
    result_ = (boost::format("Total elapsed time: %1$.2lf s  Frequency: %2$.1f Hz\n") %
               (totalTimeMs / 1000) % (count_ * 1000 / totalTimeMs))
                .str();
  }
#endif  // ENABLE_TEST_UTILITY
...
private:
  struct ProcInfo
  {
    ...
    {
      startTime_ = time;
      if (verbose_) printf("[LoopTimer] %s starts\n", name.c_str());
    }
    ...
    inline void end(const timespec & time)
    {
      ...
      if (verbose_) {
        printf("[LoopTimer] %s takes %.2lf ms\n", name.c_str(), (double)timeNs * 1e-6);
      }
    }
  };
  ...
  inline void formatResult()
  {
    ...
    if (!procs_.empty()) {
      ...
      for (current_ = procs_.begin(); current_ != procs_.end(); current_++) {
        ...
        ss << current_->name;
        if (timeMs > 10000) {
          ss << boost::format(" %1$9.2lf  s  %2$10.2lf %3$10d\n") % (timeMs / 1000) % ratio %
                  current_->count;
        } else if (timeMs > 0.01) {
          ss << boost::format(" %1$9.2lf  ms  %2$10.2lf %3$10d\n") % timeMs % ratio %
                  current_->count;
        } else if (timeMs > 0.01 * 1e-3) {
          ss << boost::format(" %1$9.2lf  us  %2$10.2lf %3$10d\n") % (timeMs * 1000) % ratio %
                  current_->count;
        } else {
          ss << boost::format(" %1$11.0lf  ns  %2$10.2lf %3$10d\n") % (timeMs * 1000000) % ratio %
                  current_->count;
        }
      }
      ss << segment;
