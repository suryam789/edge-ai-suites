    ...
    def PublishPose(self, request, context):
        ...
        try:
            ...
            for person in request.people:
                ...
                # Detailed per-person logging (from current file)
            ...
            # Nested message format (from new file)
            message = {
...
#         """Handle single pose frame (unary - more reliable)"""
#         try:
#             # Convert protobuf to dict for SSE broadcasting
#             pose_dict = {
#                 "workload_type": "3d-pose",
#                 "source_id": request.source_id,
...
#             # Extract person data with logging
#             for person in request.people:
#                 person_dict = {
#                     "person_id": person.person_id,
#                     "confidence": list(person.confidence),
...
#                     ]
#                 }
#                 pose_dict["people"].append(person_dict)
                
#                 # Compact per-person logging
#                 avg_conf = sum(person.confidence) / len(person.confidence) * 100 if person.confidence else 0
#                 print(f"[POSE] Person {person.person_id}: "
#                       f"2D={len(person.joints_2d)} joints, "
#                       f"3D={len(person.joints_3d)} joints, "
...
#             print(f"[POSE] --Frame from {request.source_id} @ {request.timestamp_ms}ms - {len(request.people)} people broadcasted")
            
#             # Broadcast to SSE clients
#             if event_loop is not None:
#                 asyncio.run_coroutine_threadsafe(
#                     sse_manager.broadcast(pose_dict), event_loop
#                 )
...
#                 # ✅ FIX: Use correct broadcast method
#                 if event_loop is not None:
#                     asyncio.run_coroutine_threadsafe(
#                         sse_manager.broadcast(pose_dict), event_loop
#                     )
